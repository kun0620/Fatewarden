 ...
 */
export type AppStage = 'local-play' | 'login' | 'menu' | 'library' | 'settings' | 'room-setup' | 'character-setup' | 'game';
...
export function computeAppStage(input: AppFlowInput): AppStage {
  const { hasSupabaseConfig, user, activeSession, pendingSession, pendingRoomSetup, pendingLibrary, pendingSettings } = input;
...
export function isGateStage(stage: AppStage): boolean {
  return stage === 'login' || stage === 'menu' || stage === 'library' || stage === 'settings' || stage === 'room-setup' || stage === 'character-setup';
}
