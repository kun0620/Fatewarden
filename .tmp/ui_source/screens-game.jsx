...
/* ---------- SESSION BANNER ---------- */
function SessionBanner({ go }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 18px", background: "linear-gradient(180deg, rgba(124,58,237,0.05), transparent)", borderBottom: "1px solid var(--border-soft)" }}>
...
function InventoryPanel({ onUse }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: 10, background: "var(--bg-deep)", border: "1px solid var(--border-soft)", borderRadius: 6 }}>
        <span style={{ color: "var(--gold)" }}>{Icon("bag", { size: 14 })}</span>
        <span style={{ fontSize: 11, color: "var(--text-3)" }}>Carry</span>
        <span style={{ flex: 1, fontFamily: "var(--f-mono)", fontSize: 11, color: "var(--text-2)", textAlign: "right" }}>34 / 90 lbs</span>
      </div>

      <div>
        <div className="fw-eyebrow" style={{ marginBottom: 6 }}>Equipped</div>
        <Item name="Staff of the Cinder-Reeve" tag="Pact weapon · 1d8 fire" icon="flame" special />
        <Item name="Leather armor" tag="AC 11 + Dex" icon="shield" />
        <Item name="Light crossbow" tag="1d8 piercing · 80/320" icon="sword" />
      </div>

      <div>
        <div className="fw-eyebrow" style={{ marginBottom: 6 }}>Carried</div>
        <Item name="Healer's potion ×2" tag="2d4+2 HP · standard" icon="potion" qty={2} action="Use" onAction={() => onUse({ kind: "use-item", item: "Healer's Potion", amount: "2d4+2 HP" })} />
        <Item name="Bone-tablet fragment" tag="Quest · Embers arc" icon="scroll" lore />
        <Item name="Brass censer-key" tag="Trinket · binding-resonance" icon="sparkles" lore />
        <Item name="Torch ×3" tag="6 hr · bright 20 ft" icon="torch" qty={3} />
        <Item name="Rations ×5" tag="5 days · 10 lbs" icon="bag" qty={5} />
        <Item name="Rope, hempen 50 ft" tag="Adventuring gear" icon="bag" />
      </div>
...
function Item({ name, tag, icon, qty, special, lore, action, onAction }) {
  return (
    <div style={{ display: "flex", gap: 8, padding: 8, marginBottom: 4, background: "var(--surface)", border: "1px solid var(--border-soft)", borderRadius: 5, alignItems: "center" }}>
      <span style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 10, color: "var(--text-3)" }}>{tag}</div>
      </div>
      {qty && <span style={{ fontFamily: "var(--f-mono)", fontSize: 10.5, color: "var(--text-2)" }}>×{qty}</span>}
      {action && <button className="fw-btn fw-btn-ghost fw-btn-sm" style={{ padding: "2px 8px", fontSize: 10 }} onClick={onAction}>{action}</button>}
    </div>
  );
}
