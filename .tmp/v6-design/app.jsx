/* global React, ReactDOM, Icon, FateLogo, FateSeal,
   ...
   GameTableScreen, CampaignLibraryScreen, SettingsScreen,
   CharacterSheetScreen, DMDashboardScreen,
   LobbyScreen, BestiaryScreen,
   CharacterVaultScreen, CharacterWizardScreen,
   CombatUIKitScreen, InventoryUIKitScreen, NarrativeUIKitScreen, MOCK */
...
const SCREENS = [
  ...
  { id: "charWizard", label: "Forge Warden",     icon: "plus"    },
  { id: "roomSetup",  label: "Room Setup",       icon: "hex"     },
  { id: "lobby",      label: "Lobby",            icon: "users"   },
  { id: "charSheet",  label: "Character Sheet",  icon: "scroll"  },
  { id: "dmDash",     label: "DM Dashboard",     icon: "crown"   },
  { id: "game",       label: "Game Table",       icon: "dice"    },
  { id: "combatKit",  label: "Combat UI Kit",    icon: "sword"   },
  { id: "invKit",     label: "Inventory UI Kit", icon: "bag"     },
  { id: "narrKit",    label: "Narrative UI Kit", icon: "scroll"  },
...
function App() {
  ...
  const META = {
    ...
    vault:      { crumb: ["Hearth", "My Characters"],      rail: "menu" },
    charWizard: { crumb: ["Hearth", "Forge a warden"],     rail: "menu" },
    lobby:      { crumb: ["Hearth", "Lobby · The Gilded Tomb"], rail: "menu" },
    charSheet:  { crumb: ["Hearth", "Character Sheet"],    rail: "char" },
    dmDash:     { crumb: ["At the Table", "DM Dashboard"], rail: "dm" },
    game:       { crumb: ["At the Table", "Session 15"],    rail: "game" },
    bestiary:   { crumb: ["The Stacks", "Bestiary"],       rail: "library" },
    combatKit:  { crumb: ["The Stacks", "Combat UI Kit"],  rail: "library" },
    invKit:     { crumb: ["The Stacks", "Inventory UI Kit"], rail: "library" },
    narrKit:    { crumb: ["The Stacks", "Narrative UI Kit"], rail: "library" },
  ...
  const screenEl = (() => {
    switch (route) {
      ...
      case "vault":     return <CharacterVaultScreen go={go} />;
      case "charWizard":return <CharacterWizardScreen go={go} />;
      case "lobby":     return <LobbyScreen go={go} />;
      case "charSheet": return <CharacterSheetScreen go={go} />;
      case "dmDash":    return <DMDashboardScreen go={go} />;
      case "game":      return <GameTableScreen go={go} />;
      case "bestiary":  return <BestiaryScreen go={go} />;
      case "combatKit": return <CombatUIKitScreen go={go} />;
      case "invKit":    return <InventoryUIKitScreen go={go} />;
      case "narrKit":   return <NarrativeUIKitScreen go={go} />;
    ...
    <>
      ...
      <div className="fw-app">
        ...
        <nav className="fw-rail">
          ...
          <RailBtn icon="scroll" label="Sheet"   active={route === "charSheet"} onClick={() => go("charSheet")} />
          <RailBtn icon="crown"  label="DM"      active={route === "dmDash"} onClick={() => go("dmDash")} />
          <RailBtn icon="skull"  label="Bestiary" active={route === "bestiary"} onClick={() => go("bestiary")} />
