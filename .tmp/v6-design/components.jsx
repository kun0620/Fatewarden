/* global React */
/* ============================================================
   FATEWARDEN — Shared components, icons, mocks
   ============================================================ */

const { useState, useEffect, useRef, useMemo, useCallback } = React;

/* ---------- ICONS ----------
   Stroke-based, 20px viewBox, currentColor.
*/
const I = {};
const mk = (name, paths) => {
...
mk("mail",  <><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 7 9-7"/></>);
...
/* ---------- BRAND MARK ---------- */
function FateSeal({ size = 64, animate = true }) {
  return (
...
/* ---------- LITTLE PRIMITIVES ---------- */
function Card({ elev, children, className = "", style }) {
  return (
    <div className={`fw-card ${elev ? "fw-card-elev" : ""} ${className}`} style={style}>{children}</div>
  );
}
function CardHead({ icon, title, right, glow }) {
  return (
...
function Field({ label, hint, children, right }) {
  return (
    <div className="fw-field">
      ...
      {children}
      {hint && <div style={{ fontSize: 11.5, color: "var(--text-3)", fontStyle: "italic", fontFamily: "var(--f-serif)" }}>{hint}</div>}
    </div>
  );
}
    ...
    <div className="fw-seg">
      {options.map(o => (
        <button key={o.value || o}
          className={"fw-seg-btn " + ((o.value || o) === value ? "active" : "")}
...
function Toggle({ on, onChange }) {
  return <div className={"fw-toggle " + (on ? "on" : "")} onClick={() => onChange(!on)} />;
}
function Tile({ active, onClick, title, desc, icon, children }) {
  return (
...
const RACES = ["Human","Elf","Dwarf","Halfling","Tiefling","Half-Orc","Dragonborn","Gnome"];
...
Object.assign(window, {
  Icon, I, FateSeal, FateLogo,
  Card, CardHead, Field, Seg, Toggle, Tile,
  MOCK, CLASSES, RACES, BACKGROUNDS,
});
